...
errno_t GPU_SVD_computeControlMatrix(int device, const char *ID_Rmatrix_name, const char *ID_Cmatrix_name,
                                     double SVDeps, const char *ID_VTmatrix_name)
{
    DEBUG_TRACE_FSTART();
    ...
    cublasStatus_t cublasStat =
        cublasSgemm(cublasH, CUBLAS_OP_T, CUBLAS_OP_T, n, m, n, &alpha, d_VT, n, d_U, m, &beta, d_M, n);
    ...
    FUNC_CHECK_RETURN(create_image_ID(ID_Cmatrix_name, data.image[ID_Rmatrix].md[0].naxis, arraysizetmp,
                                      _DATATYPE_FLOAT, 0, 0, 0, &ID_Cmatrix));
