...
static errno_t CUDACOMP_test_cli()
{
    ...
    {
        GPUcomp_test(data.cmdargtoken[1].val.numl, data.cmdargtoken[2].val.numl, data.cmdargtoken[3].val.numl,
...
{
    RegisterCLIcommand("cudacomptest", __FILE__, CUDACOMP_test_cli, "test CUDA comp",
...
errno_t GPUcomp_test(__attribute__((unused)) long NBact, long NBmodes, long WFSsize, long GPUcnt)
{
    ...
    GPU_loop_MultMat_setup(0, data.image[ID_contrM].name, data.image[ID_WFS].name, data.image[ID_cmd_modes].name,
